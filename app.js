...
const sanitizeTurnoConfigForStorage = (turno, rawConfig = {}) => {
  ...
  return {
    ...
    horaFin: safeString(rawConfig.horaFin || defaults.horaFin),
    dias: diasString,
...
const getTurnoEndMinutes = (cfg = {}) => parseTimeToMinutes(cfg.horaFin || DEFAULT_HORA_FIN);

const getBloquesMinimosParaVista = (turno, cfg) => {
  const endMinutes = getTurnoEndMinutes(cfg);
  if (!Number.isInteger(endMinutes)) return 1;

  const inicio = parseTimeToMinutes(cfg.horaInicio || DEFAULT_HORA_INICIO);
...
const validateFutureAssignmentRules = ({
  ...
  const cargaActualDia = cargaPorDia[dayIndex] || 0;
  const maxDiarioPorCapacidad = Math.min(maxBloquesPorDia, slotsDisponiblesPorDia[dayIndex] || 0);
  if ((cargaActualDia + bloquesAAgregar) > maxDiarioPorCapacidad) {
    return { valid: false, reason: 'Se alcanzó el máximo de bloques por día para la carrera.' };
  }

  const minCargaActual = Math.min(...cargaPorDia);
  const projectedDayCount = cargaActualDia + bloquesAAgregar;
  const existeDiaMenosCargadoConCapacidad = cargaPorDia.some((dayCount, index) => {
    if (index === dayIndex) return false;
    const capacidadDia = Math.min(maxBloquesPorDia, slotsDisponiblesPorDia[index] || 0);
    return dayCount === minCargaActual && dayCount < capacidadDia;
  });

  if (existeDiaMenosCargadoConCapacidad && projectedDayCount > (minCargaActual + bloquesAAgregar)) {
...
const encontrarHuecoContiguo = (slotsDisponibles, bloquesNecesarios) => {
  ...
  for (let start = 0; start <= (disponibles.length - requeridos); start += 1) {
    ...
    if (consecutivos) return tramo;
  }

  return [];
};

const asignarClaseEnSlots = (clase, slots = []) => {
...
const generarPlanHorario = ({ turno, clases = [] }) => {
  ...
  const maxBloquesPorDia = Math.max(toPositiveNumber(getTurnoConfig(turno).maxTurnos, 4), 1);
  const ocupacionPorSlot = buildOcupacionPorSlot(totalSlots);
  const cargaPorDia = Array.from({ length: diasCount }, () => 0);
  ...
  clasesEnOrden.forEach((candidate, classIndex) => {
    ...
    for (let orderIndex = 0; orderIndex < dayOrder.length; orderIndex += 1) {
      ...
      const capacidadDia = Math.min(maxBloquesPorDia, slotsDisponiblesPorDia[dayIndex] || 0);
      if ((cargaPorDia[dayIndex] || 0) >= capacidadDia) {
        ultimoMotivoRegla = 'Se alcanzó el máximo de bloques por día para la carrera.';
        continue;
      }
      ...
      const huecoContiguo = encontrarHuecoContiguo(slotsDia, bloquesNecesarios);
      ...
      const rules = validateFutureAssignmentRules({
        ...
        maxBloquesPorDia,
        cargaPorDia,
        bloquesAAgregar: bloquesNecesarios,
        slots,
        slotsDisponiblesPorDia,
      });
    ...
    asignarClaseEnSlots(candidate, slotsObjetivo);
    slotsAsignables.forEach((slotIndex) => marcarOcupacion(candidate, slotIndex, ocupacionPorSlot));

    if (diaAsignado >= 0) cargaPorDia[diaAsignado] = (cargaPorDia[diaAsignado] || 0) + bloquesNecesarios;
  });
